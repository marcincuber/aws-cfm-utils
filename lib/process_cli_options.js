...
const processopts = (argv) => {
  ...
  if (argexists('sessiontoken')) {
    args.sessiontoken = argstring('sessiontoken');
  }
  ...
  if (argexists('suspend-scheduled-actions')) {
    args.suspendScheduledActions = argboolean('suspend-scheduled-actions');
  }
  if (argexists('refresh-rate')) {
    args.refreshRate = argint('refresh-rate');
  }
